...
export class ProjectsController {
  ...
  /**
   * Ensure we have a local User row for this Auth0 user,
  ...
  async listProjects(
    ...
    const projects = await this.projectsService.listByWorkspace(
      workspaceId,
      userId,
    );
    return { projects };
  ...
  // Permanent delete with RBAC (OWNER / ADMIN only)
  @Delete(':id')
  @UseGuards(WorkspaceRoleGuard)
  @WorkspaceRoles(WorkspaceRole.OWNER, WorkspaceRole.ADMIN)
  async deleteProject(
